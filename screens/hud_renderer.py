...
def render_button(
    ...
    label = font.render(text, True, config.WINDOW.color_fg)
...
def render_jostick(screen):
    ...
    pygame.draw.circle(
        screen, (100, 100, 100), (center_x, center_y), base_radius
    )

    pygame.draw.line(
        screen, (0, 0, 0), (center_x, center_y), (end_x, end_y), 6
    )
        ...
        while True:
            ...
            for button in config.button_states.keys():
                ...
                render_button(
                    self.screen,
                    self.font,
                    config.BUTTONS_POSITION[button],
                    button,
                    state["state"],
                )
